...
def create_report(url, page, article, all_locations):
    all_diseases = populating_diseases_list()
    ...
    diseases = get_disease(article['headline'], all_diseases)
    report_obj['diseases'] = diseases
    locations = get_location(article['headline'], article['main_text'], url)
    #print(locations)
    # set of two lists
    ...
    report_obj['event_date'] = parser.isoparse(str(get_eventDate(article['main_text'], article['date_of_publication'], article['headline'])))
    

    single_report.append(report_obj)
    return single_report

def populating_diseases_list():
    # Populating all the lists 
...
def main(requests):
    ...
    # Initializing all the lists we need to create
    link_list = []
    ...
    # Create article along with the individual report associated with thus 
    article = create_article(url, page)
    reports  = create_report(url, page, article,all_locations)
